...
#if defined(NDEBUG)
#error "DigitalDenar cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uDDEN) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < 1)
        ...
        nSubsidy = 100 * COIN;
    else if (nHeight < 250201)
        nSubsidy = 50 * COIN;
    else if (nHeight < 650201)
        nSubsidy = 25 * COIN;
    else
        nSubsidy = 10 * COIN;
...
{
    RenameThread("digitaldenar-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // DDEN: We use certain sporks during IBD, so check to see if they are
